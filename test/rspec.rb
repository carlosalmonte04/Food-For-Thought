require_relative "../rails_helper.rb"
  ...
  def create_user
    user_info = {
      ...
      first_name: "Carlos",
      last_name: "Almonte",
      email: "CLO4EVER@email.com",
      small_picture: https://static.pexels.com/photos/126407/pexels-photo-126407.jpeg,
      profile_picture: https://static.pexels.com/photos/126407/pexels-photo-126407.jpeg,
      slack_id: "carlos-almonte"
      }
  ...
  def user_login
    fill_in("session[id]", :with => @user.id)
    click_button("commit")
  end
...
describe 'Feature Test: User Signout', :type => :feature do
  ...
  it 'lets users log out from the index page' do
    before do
      user_login
      set_session
      visit_index_page
...
describe 'Feature Test: New Post', :type => :feature do

  it 'successfully lets a user create a post' do
    before do
      user_login
      set_session
      visit_index_page
    ...
    click_link("New Post")
    fill_in("post[title]", :with => "Can somebody help me?")
    fill_in("post[topic_attributes][id]", :with => "2")
    fill_in("post[description]", :with => "Need help very badly with this topic! Help help haaaaalp!")
    fill_in("post[length]", :with => "90 minutes")
    fill_in("post[deadline]", :with => "2017-09-09")
    fill_in("post[picture]", :with => "https://static.pexels.com/photos/127028/pexels-photo-127028.jpeg")
  end

describe 'Feature Test: Filter', :type => :feature do

  it 'successfully filters out ' do
    before do
      user_login
      set_session
      visit_index_page
...
##########
  it "on sign in, successfully adds a session hash" do
    user_login
    user_signup
